...
인덱스 카드에서 중요도(★ 1~5), 도메인(Art / Science / Philosophy), 태그를 바로 고칠 수 있다.
...
타입과 도메인 두 줄, 그리고 Temporal 뷰의 타임라인. 셋은 서로를 덮어쓰지 않는다 —
노드의 가시성은 한 곳에서 세 조건을 모두 확인해 결정한다. 마지막 하나를 끄면 빈 화면
대신 전체가 돌아온다.
